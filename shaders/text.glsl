...
vs2fs vec4 vs_text_col;
	...
	out vec4 frag_col;
	void main () {
		float alpha = texture(atlas_tex, vs_uv).r;
		frag_col = vs_text_col * (vs_text_col.a * alpha);
	}
#else
	...
	out vec4 frag_col;
	void main () {
		vec2 fw = fwidth(vs_uv) * vec2(textureSize(atlas_tex, 0)); // texels / screen pixel 
		float scale = 2.0 / (fw.x + fw.y);
		
		
		float sdf = ((texture(atlas_tex, vs_uv).r - sdf_onedge) * sdf_scale);
		sdf += sdf_grow;
		...
		frag_col = sdf_outline_col * (sdf_outline_col.a * (oalpha - ialpha))
		             + vs_text_col * (    vs_text_col.a *  ialpha          );
	}
#endif
